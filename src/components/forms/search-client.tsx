      ...
      <div className="mx-auto max-w-2xl">
        <div className="relative">
          ...
          <input
            ...
            className="h-13 w-full rounded-[12px] border border-line bg-white py-3.5 pl-11 pr-4 text-[15px] text-ink placeholder:text-ink-soft focus:border-brand-700 focus:outline-none focus-visible:shadow-[0_0_0_3px_rgba(122,36,50,0.18)]"
          />
        </div>
      </div>
        ...
        {(["all", ...searchTypes] as const).map((t) => (
          <button
            ...
            className={cn(
              ...
              type === t ? "border-brand-700 bg-brand-700 text-white" : "border-line bg-white text-ink hover:border-brand-700",
            )}
          >
            ...
            <ul className="space-y-3">
              {results.map((r) => (
                <li key={`${r.type}-${r.href}-${r.title}`}>
                  <Link href={r.href} className="block rounded-[12px] border border-line bg-white p-4 transition-colors hover:border-brand-700">
