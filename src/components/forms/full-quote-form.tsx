...
const fieldBase =
  "h-12 w-full rounded-[10px] border bg-white px-3.5 text-[15px] text-ink placeholder:text-ink-soft focus:border-brand-700 focus:outline-none focus-visible:shadow-[0_0_0_3px_rgba(122,36,50,0.18)]";
    ...
    <div className={cn("flex flex-wrap gap-2", errored && "rounded-[10px]")}>
      {options.map((o, i) => (
        <label key={o.v} className="cursor-pointer">
          ...
          <span className="inline-block rounded-[10px] border border-line bg-white px-4 py-2 text-sm text-ink peer-checked:border-brand-700 peer-checked:bg-brand-700 peer-checked:text-white">
            {o.l}
          </span>
        </label>
      ))}
...
function SubmitButton({ disabled }: { disabled?: boolean }) {
  ...
  return (
    <button
      ...
      className="flex h-12 items-center justify-center gap-2 rounded-[10px] bg-brand-700 px-8 text-[15px] font-semibold text-white hover:bg-brand-800 disabled:opacity-70"
    >
    ...
    <div>
      ...
      <form ref={formRef} action={action} noValidate>
          ...
          <div className="mt-5 space-y-5">
            ...
            <div>
              ...
              <textarea id="q-desc" name="damageDescription" rows={4}
                className={cn("w-full rounded-[10px] border bg-white p-3.5 text-[15px] text-ink placeholder:text-ink-soft focus:border-brand-700 focus:outline-none", errors.damageDescription && "border-error")} />
        ...
        <div className={cn(step === 2 ? "block" : "hidden")}>
          ...
          <label className="mt-5 flex cursor-pointer flex-col items-center justify-center gap-2 rounded-[14px] border-2 border-dashed border-line-strong bg-cream-50 px-6 py-10 text-center hover:border-brand-700">
        ...
        <div className={cn(step === 4 ? "block" : "hidden")}>
          ...
          <div className="mt-5 space-y-3">
            <label className="flex items-start gap-2.5 text-[13px] leading-snug text-ink-secondary">
              <input type="checkbox" name="consent" className="mt-0.5 h-4 w-4 shrink-0 accent-brand-700" />
              <span>
                <a href={routes.kvkk} className="font-medium text-brand-700 underline">KVKK Aydınlatma Metni</a>
                ’ni okudum; bilgilerimin değerlendirme amacıyla işlenmesini onaylıyorum.
              </span>
            </label>
            <Err msg={errors.consent} />
            <label className="flex items-start gap-2.5 text-[13px] leading-snug text-ink-secondary">
              <input type="checkbox" name="marketing" className="mt-0.5 h-4 w-4 shrink-0 accent-brand-700" />
        ...
        <div className="mt-8 flex items-center justify-between gap-3">
          ...
          {isLast ? (
            ...
            <button type="button" onClick={goNext} className="inline-flex items-center gap-1.5 rounded-[10px] bg-brand-700 px-6 py-3 text-sm font-semibold text-white hover:bg-brand-800">
