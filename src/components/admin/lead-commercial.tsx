...
const field = "h-10 rounded-md border border-line px-3 text-sm focus:border-brand-700 focus:outline-none";
      ...
      <div className="flex items-center justify-between border-b border-line px-5 py-3.5">
        ...
        {bundle.deal ? (
          ...
          <form action={createDealForLead}>
            ...
            <button type="submit" className="rounded-md bg-brand-700 px-3 py-1.5 text-xs font-semibold text-white hover:bg-brand-800">
              {labels.create}
            </button>
          </form>
        ) : null}
        ...
        <div>
          ...
          {perms.referWrite && buyers.length > 0 && (
            <form action={referToBuyers} className="space-y-2">
              ...
              <button type="submit" className="rounded-md bg-brand-700 px-4 py-2 text-sm font-semibold text-white hover:bg-brand-800">{labels.add}</button>
            </form>
          )}
              ...
              <li key={o.id} className="flex items-center justify-between gap-2">
                ...
                {!o.selected && perms.offerWrite && (
                  <form action={selectBuyerOffer}>
                    ...
                    <input type="hidden" name="leadId" value={leadId} />
                    <button type="submit" className="text-xs font-semibold text-brand-700 hover:underline">{labels.select}</button>
                  </form>
                )}
              </li>
            ))}
