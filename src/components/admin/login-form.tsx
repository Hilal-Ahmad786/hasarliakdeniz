...
function SubmitButton({ label }: { label: string }) {
  ...
  return (
    <button
      type="submit"
      disabled={pending}
      className="flex h-12 w-full items-center justify-center gap-2 rounded-[10px] bg-brand-700 text-[15px] font-semibold text-white transition-colors hover:bg-brand-800 disabled:opacity-70"
    >
    ...
    <form action={action} className="space-y-4">
      ...
      <div>
        ...
        <input
          ...
          autoComplete="email"
          required
          className="h-12 w-full rounded-[10px] border border-line bg-white px-3.5 text-[15px] text-ink focus:border-brand-700 focus:outline-none focus-visible:shadow-[0_0_0_3px_rgba(122,36,50,0.18)]"
        />
      </div>
      <div>
