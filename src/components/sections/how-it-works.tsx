        ...
        {steps.map((s, i) => (
          <div key={s.title} className="relative flex flex-col items-center text-center">
            <div className="relative grid h-[72px] w-[72px] place-items-center rounded-full border border-line bg-cream-50">
              <s.icon size={30} className="text-brand-700" strokeWidth={1.8} />
              <span className="absolute -right-1 -top-1 grid h-7 w-7 place-items-center rounded-full bg-brand-700 text-sm font-bold text-white">
                {i + 1}
              </span>
            </div>
