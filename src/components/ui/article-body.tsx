    ...
    <div className="space-y-5">
      {blocks.map((b, i) => {
        switch (b.type) {
          ...
          case "ul":
            return (
              <ul key={i} className="list-disc space-y-2 pl-5 text-[17px] leading-relaxed text-ink-secondary marker:text-brand-700">
                {b.items.map((it, j) => <li key={j}>{renderInline(it)}</li>)}
              </ul>
            );
          case "ol":
            return (
              <ol key={i} className="list-decimal space-y-2 pl-5 text-[17px] leading-relaxed text-ink-secondary marker:font-semibold marker:text-brand-700">
                {b.items.map((it, j) => <li key={j}>{renderInline(it)}</li>)}
              </ol>
            );
          case "note":
