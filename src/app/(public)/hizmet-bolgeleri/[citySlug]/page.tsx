...
export default async function CityPage({
  ...
  const cityFaqs = content?.faqs ?? [
    { q: `${city.locative} hangi araçlar alınıyor?`, a: `${city.locative} hasarlı, kazalı, pert, arızalı, çalışmayan, yanmış, sel hasarlı, hurda ve çekme belgeli araçlar için ücretsiz değerlendirme talebi oluşturabilirsiniz.` },
    { q: "Değerlendirme için ücret ödüyor muyum?", a: "Hayır. Ön değerlendirme ve teklif tamamen ücretsizdir ve sizi bağlamaz." },
  ];
    ...
    <>
      ...
      {content && (
        <Section tone="white">
          <div className="max-w-[760px]">
            <SectionHeading eyebrow="Sahayı Tanıyoruz" title={`${city.locative} Alım Süreci Nasıl İşliyor?`} align="left" />
      ...
      <Section tone="cream">
        ...
        <div className="mt-9 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
          {services.map((s) => (
            <Link key={s.slug} href={routes.service(s.slug)} className="group flex items-center gap-3 rounded-[12px] border border-line bg-white p-3 transition-colors hover:border-brand-700">
      ...
      {cityDistricts.length > 0 && (
        <Section tone="white">
          ...
          <div className="mt-6 flex flex-wrap gap-2.5">
            {cityDistricts.map((d) => (
              <Link key={d.slug} href={routes.district(citySlug, d.slug)} className="inline-flex items-center gap-1.5 rounded-full border border-line bg-cream-50 px-4 py-2 text-sm font-medium text-ink hover:border-brand-700 hover:text-brand-700">
      ...
      {nearby.length > 0 && (
        <Section tone="white">
          ...
          <div className="mt-6 flex flex-wrap gap-2.5">
            {nearby.map((c) => (
              <Link key={c.slug} href={routes.city(c.slug)} className="inline-flex items-center gap-1.5 rounded-full border border-line bg-cream-50 px-4 py-2 text-sm font-medium text-ink hover:border-brand-700 hover:text-brand-700">
                {c.name}
                <ArrowRight size={14} />
              </Link>
            ))}
