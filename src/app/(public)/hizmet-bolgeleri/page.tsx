  ...
  return (
    <>
      ...
      <PageHero
        ...
        eyebrow="Akdeniz Bölgesi Araç Alım Hizmeti"
        title="Aracınızın Bulunduğu Şehri Seçin"
        description="Akdeniz Bölgesi genelinden başvuru kabul edilir. Hizmet uygunluğu, araç ve konum değerlendirmesinin ardından netleştirilir."
      >
        ...
        <div className="mt-9 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
          {featuredCities.map((c) => (
            <Link
              key={c.slug}
              href={routes.city(c.slug)}
              className="group flex items-center gap-3 rounded-[14px] border border-line bg-white p-4 transition-colors hover:border-brand-700"
            >
              <span className="grid h-10 w-10 shrink-0 place-items-center rounded-[10px] bg-cream-100 text-brand-700">
                <MapPin size={18} />
              </span>
