    ...
    <>
      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          {lead && <Link href={`/admin/leads/${lead.id}`} className="text-sm text-brand-700 hover:underline">{lead.leadNumber}</Link>}
        </div>
        ...
        <div className="lg:col-span-2">
          {canWrite ? (
            <form action={updateDeal} className="rounded-[14px] border border-line bg-white p-6">
              ...
              <button type="submit" className="mt-5 rounded-md bg-brand-700 px-6 py-2.5 text-sm font-semibold text-white hover:bg-brand-800">
                {t("common.save")}
              </button>
            </form>
          ) : (
