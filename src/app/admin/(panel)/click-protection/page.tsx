        ...
        <div className="rounded-[14px] border border-line bg-white p-5">
          ...
          <form action={saveAvgCpc} className="mt-3 flex items-center gap-2">
            <input
              ...
              className="h-10 w-40 rounded-md border border-line px-3 text-sm focus:border-brand-700 focus:outline-none disabled:bg-cream-50"
            />
            <span className="text-sm text-ink-muted">₺ / tıklama</span>
            {manage && (
              <button className="rounded-md bg-brand-700 px-4 py-2 text-sm font-semibold text-white hover:bg-brand-800">
                Kaydet
              </button>
            )}
          </form>
            ...
            {series.map((s) => (
              <div key={s.day} className="flex items-center gap-3 text-xs">
                ...
                <div className="relative h-4 flex-1 overflow-hidden rounded bg-cream-100">
                  <div className="absolute inset-y-0 left-0 bg-brand-700/30" style={{ width: `${(s.clicks / maxDay) * 100}%` }} />
            ...
            return (
              <div key={hour} className="flex flex-1 flex-col items-center gap-1">
                <div className="flex w-full flex-1 items-end">
                  <div className="relative w-full overflow-hidden rounded-t bg-brand-700/30" style={{ height: `${(clicks / maxHour) * 100}%` }}>
