            ...
            >
              <div className="flex items-start justify-between gap-2">
                <Link
                  ...
                  className="text-sm font-bold leading-snug text-ink hover:text-brand-700"
                >
              ...
              <p className={`mt-1 text-3xl font-bold ${isLost ? "text-error" : "text-brand-700"}`}>
                {items.length}
              </p>
              <div className="mt-3 space-y-1.5">
                {items.slice(0, 3).map((l) => (
                  <Link
                    ...
                    className="block truncate text-xs text-ink-secondary hover:text-brand-700"
                  >
                    {l.fullName}
                    {l.city ? ` · ${l.city}` : ""}
                  </Link>
                ...
                {items.length > 3 && (
                  <Link href={`/admin/leads?stage=${stage}`} className="block text-xs font-semibold text-brand-700 hover:underline">
                    +{items.length - 3} daha
                  </Link>
                )}
            ...
            return (
              <div key={lvl.start} className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  <div
                    className="mx-auto flex h-10 min-w-[2.5rem] items-center justify-center rounded-md bg-gradient-to-r from-brand-800 to-brand-600 px-3 text-sm font-bold text-white transition-all"
