        ...
        {VIEWS.map((v) => (
          <Link
            ...
            className={cn(
              ...
              view === v ? "border-brand-700 bg-brand-700 text-white" : "border-line bg-white text-ink hover:border-brand-700",
            )}
          >
            {t(`view.${v}`)}
          </Link>
            ...
            return (
              <Link
                ...
                className={cn(
                  ...
                  active ? "border-brand-700 bg-cream-100" : "border-line bg-white hover:border-brand-700",
                )}
              >
      ...
      <form method="get" className="mb-5 flex flex-wrap items-end gap-3 rounded-[14px] border border-line bg-white p-4">
        ...
        <div className="min-w-[180px] flex-1">
          ...
          <input name="q" defaultValue={sp.q ?? ""} placeholder="Ad, telefon, no…"
            className="h-10 w-full rounded-md border border-line px-3 text-sm focus:border-brand-700 focus:outline-none" />
        </div>
        <div>
        ...
        <button type="submit" className="h-10 rounded-md bg-brand-700 px-5 text-sm font-semibold text-white hover:bg-brand-800">
          {t("common.search")}
        </button>
      </form>
              ...
              data.rows.map((l) => (
                <tr key={l.id} className="hover:bg-cream-50">
                  <td className="px-4 py-3">
                    <Link href={`/admin/leads/${l.id}`} className="font-semibold text-brand-700 hover:underline">{l.fullName}</Link>
