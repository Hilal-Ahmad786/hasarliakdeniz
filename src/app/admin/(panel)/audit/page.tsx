  ...
  return (
    <>
      ...
      <form method="get" className="mb-4 flex items-end gap-2">
        ...
        <button type="submit" className="h-10 rounded-md bg-brand-700 px-5 text-sm font-semibold text-white hover:bg-brand-800">{t("common.search")}</button>
      </form>
            ...
            {data.rows.length === 0 ? (
              ...
              <tr key={r.id} className="hover:bg-cream-50">
                ...
                <td className="px-4 py-3 text-ink-secondary">
                  {r.entityType ? (r.entityType === "lead" && r.entityId ? <Link href={`/admin/leads/${r.entityId}`} className="text-brand-700 hover:underline">{r.entityType}</Link> : r.entityType) : "—"}
                </td>
