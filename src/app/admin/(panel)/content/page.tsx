  ...
  return (
    <>
      <div className="mb-5 flex items-center justify-between">
        ...
        <Link href="/admin/content/new" className="inline-flex items-center gap-1.5 rounded-md bg-brand-700 px-4 py-2 text-sm font-semibold text-white hover:bg-brand-800">
        ...
        {["", "blog_post", "guide", "page", "service", "faq"].map((type) => (
          <Link
            ...
            className={cn(
              ...
              (sp.type ?? "") === type ? "border-brand-700 bg-brand-700 text-white" : "border-line bg-white text-ink hover:border-brand-700",
            )}
          >
              ...
              rows.map((c) => (
                <tr key={c.id} className="hover:bg-cream-50">
                  <td className="px-4 py-3">
                    <Link href={`/admin/content/${c.id}`} className="font-semibold text-brand-700 hover:underline">{c.title}</Link>
